...
def _get_api_key(user_input: str) -> str:
...
def build_kpi_chart(fields: dict) -> go.Figure:
    ...
    # Vertical line at reference month — use add_shape to avoid Plotly
    # string-axis bug with add_vline on categorical/string x-axes
    fig.add_shape(
        type="line", xref="x", yref="paper",
        x0=ref_month, x1=ref_month, y0=0, y1=1,
        line=dict(color="#d4a017", width=1.5, dash="dash"),
    )
    fig.add_annotation(
        x=ref_month, yref="paper", y=0.97,
        text=f"Senaste justering<br>{ref_month}",
        showarrow=False,
        xanchor="right", font=dict(size=11, color="#d4a017"),
    ...
    fig.add_shape(
        type="line", xref="x", yref="paper",
        x0=CURRENT_MONTH, x1=CURRENT_MONTH, y0=0, y1=1,
        line=dict(color="#c0392b", width=1.5, dash="dot"),
    )
    fig.add_annotation(
        x=CURRENT_MONTH, yref="paper", y=0.97,
        text="Idag",
        showarrow=False,
        xanchor="left", font=dict(size=11, color="#c0392b"),
        bgcolor="rgba(255,255,255,0.8)",
    )
...
with st.sidebar:
    ...
    api_key_input = st.text_input(
        ...
        help="Valfritt om ANTHROPIC_API_KEY finns i Streamlit Secrets eller miljövariabler.",
    )
...
if demo_mode:
    ...
    if not api_key:
        ...
        # Debug: show masked key source so users can confirm the right key is loaded
        masked = api_key[:10] + "..." + api_key[-4:] if len(api_key) > 14 else "***"
        source = "sidebar" if (api_key_input and api_key_input.strip()) else "secrets/env"
        with st.expander(f"🔑 API-nyckel laddad ({source}): `{masked}`  — expandera för felsökning"):
            st.markdown(f"""
**Nyckelkälla:** `{source}`  
**Maskerad nyckel:** `{masked}`  
**Längd:** `{len(api_key)}` tecken  

Om nyckeln ser fel ut, kontrollera:
- Streamlit Cloud → Settings → Secrets → värdet ska vara `sk-ant-...` utan extra citattecken
- Nyckeln ska börja med `sk-ant-`
- Kopiera nyckeln direkt från [console.anthropic.com](https://console.anthropic.com)
""")
